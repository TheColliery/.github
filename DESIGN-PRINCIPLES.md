...
| System | What it is |
|---|---|
| **1. Config** | Every user-tunable value is exposed in a `.<tool>.json` cascade (project overrides global) — a single source of truth, CLI-settable, verify-validated. Nothing load-bearing is hard-coded out of reach. |
