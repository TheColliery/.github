...
| System | What it is |
|---|---|
| **1. Config** | Every user-tunable value lives in a **two-level `.<tool>.json` cascade** — a global `~/.claude/.<tool>.json` overlaid by the nearest per-project `.<tool>.json` (project wins) — schema-validated against one source of truth, range-clamped on read, prototype-pollution-guarded, home-stopped (symlink-correct). **The payoff: a globally-installed tool can be re-tuned or shut OFF per project** — a skill that doesn't fit a given project need never keep loading (and burning tokens) there. Nothing load-bearing is hard-coded out of reach. |
