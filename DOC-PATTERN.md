...
- **Em dashes take no surrounding spaces** (`word—word`, never `word — word`)—owner ruling 2026-09-03, the Google/Microsoft/Chicago camp: the house is developer documentation, the genre those style guides are written for. The flock's older spaced form is retired; a one-shot sweep conforms each repo once, on its own belt (`.github` itself swept under UMB-042)—an unswept repo's old spaced dashes are known debt, not a new decision, until its own sweep lands. Applies to prose only: a fenced code block, an inline code span, or a verbatim quotation keeps its own bytes exactly as written.
...
**The spine has two layers (SERIES-CANON.md's L1/L2 split, applied to README rows).** Rows 1–5 and 7, 10–11 are the L1 floor every published-code repo carries: a title and one-line description, what it is, how it works, compatibility, install, configure, the series block and the licence, plus the community files (Standard Readme's required set; install and usage are optional only for a repo with no functional code). Rows 6 (Commands), 8 (Permissions) and 9 (Benchmark) belong to the Coal* skill overlay, L2. **A published-code repo that ships no skill (Kolwen) either carries them or declares each N/A in one line under its status line—MUST: a missing spine row is declared, never silent** (the same N/A-by-declaration SERIES-CANON.md already uses for an absent spine document). <!-- coalmine: verified 2026-09-24 · exemplar Standard Readme spec · revalidate 90d -->
...
| 2 | Reporting a Vulnerability | **Route to GitHub's private vulnerability reporting FIRST**—`Security → Report a vulnerability` on the repo's Security tab, i.e. `<repo-url>/security/advisories/new`—**never a bare "open an issue."** State the **SCOPE** (what's in-bounds; a room with a real destructive surface names its own high-value attack class explicitly—CoalWash's undo/bins-blast-radius paragraph is the pattern, a room-specific extension, not drift) and what a reporter can **EXPECT** (read + acknowledged, triaged against the scope, disclosed once a fix ships, no fixed-SLA for a solo-maintainer project). A plain **public GitHub issue stays correct for an ordinary, non-security bug**—name it as that fallback, never as an alternate route for a vulnerability. *(Locked 2026-09-02, CWK-052—replaces the old "open an issue / request a private channel for sensitive PoC" wording, which named no address for the private half and let every shipped `SECURITY.md` route a real vulnerability report to a public issue.)* |
...
| 6 | Releasing (Maintainers) | The bump → CHANGELOG → green gates → signed tag → push → GitHub Release (stable tags only, plus a first pre-release version's one launch-form Release) chain. |
...
- **Each version heading resolves to its diff (prefer).** A link reference at the file's foot, `[X.Y.Z]: https://github.com/<org>/<repo>/compare/vPREV...vX.Y.Z`, with `[Unreleased]` comparing the last tag to `HEAD` (Keep a Changelog 1.1.0: "Versions and sections should be linkable"). Depart for a repo with no public tags, or a private repo where a compare link resolves only for authenticated readers, and say so in the file. <!-- coalmine: verified 2026-09-24 · exemplar Keep a Changelog 1.1.0 · revalidate 90d -->
...
GitHub serves six community files from the org's public `.github` repository to every repo that ships none of its own (root, `.github/` or `docs/`; `.github/` first). They live in `.github/` so the root stays lean and each is deliberately short: a long explanation is a link. A room's own file **wins**, a named divergence that `scripts/skeleton-check.mjs` reports (`inherits the org default` when absent, `NAMED DIVERGENCE` when it differs).
...
| `GOVERNANCE.md` | Who decides (one maintainer), how a change is accepted (the repository's gates and rulesets), who writes the changes (AI agents under the maintainer's review; the `Co-Authored-By` trailer is the record), where to report. The public summary of the maintainer's decision rights, never a copy of the private rules. A room with its own governance ships its own file (the article kind does). |
...
| Website field | The org landing (`github.com/TheColliery`) unless the tool has a better front door. A docs site is linked by the stable `https://thecolliery.org/docs`, never by a GitBook host (`skeleton-check.mjs --details` fails a website that names one). |
| Releases panel | Never empty on a stable tool (= reads abandoned); the latest stable visible. Policy: tags = beta+stable, Releases = stable-only, plus the ONE launch-form pre-release Release a repo's first public beta/rc gets (RELEASE-PATTERN.md "Which tags get a Release"). |
