...
| 3 | Commit & Tag Signatures | SSH-signed (`gpg.format=ssh`); GitHub renders the Verified badge. The local-verify snippet MUST be self-contained and actually run: it MUST first CREATE the allowed-signers file — `echo "* ssh-ed25519 <key>" > <tool>_signers` — *before* `git config gpg.ssh.allowedSignersFile ./<tool>_signers`, because `git verify-commit HEAD` FAILS without it (`allowedSignersFile needs to be configured and exist`). Use the `*` principal (matches any committer email → verifies the KEY, not a hardcoded identity; never invent a principal email — it must match the signer or verify says "No principal matched"). Then `git verify-commit HEAD && git tag -v "$(git describe --tags --abbrev=0)"`. The `echo`+`config` lines are LOAD-BEARING DATA — never trim them as "fat" (doing so silently broke all three repos' verify until 2026-06-22). |
