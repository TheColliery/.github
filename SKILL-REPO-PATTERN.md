...
All workflows **SHA-pinned** (40-char, with a `# vX` comment): `ci.yml` (the green gate on push/PR) · `codeql.yml` · `markdownlint.yml` · `scorecard.yml` · `coverage.yml` (report-only, below) · `link-check.yml` (below) · `dependabot-auto-merge.yml` (org-canonical since 2026-07-09—actor-guarded to `dependabot[bot]` only, patch/minor only, `gh pr merge --auto` behind a required-checks ruleset; a MAJOR bump and any human-opened PR still wait for the human). Plus `dependabot.yml` and `ISSUE_TEMPLATE/` (`bug-report.yml` + `config.yml`; add `platform-report.yml` when the tool is cross-agent). Issue templates that name a version carry a `version-pin:` marker so `verify.mjs` catches a stale pin.
...
- **Two more rulesets are MUST on every PUBLIC repo, both with an EMPTY bypass list (rulesets canon, owner 2026-09-24).** `main-guard` = a branch ruleset on `~DEFAULT_BRANCH` with the rules `deletion` + `non_fast_forward`, enforcement `active`: nobody deletes or force-pushes the default branch with a token, and a deliberate history rewrite means the owner disables the ruleset in the UI first, then re-enables it. `tag-immutable` = a tag ruleset on `refs/tags/**` with the rules `update` + `deletion` + `non_fast_forward`, enforcement `active`: a published tag cannot be moved, deleted or force-pushed, while CREATING a tag stays free (the release flow pushes new tags with `--follow-tags`). The `dependabot-auto-merge-gate` ruleset above is a different ruleset and KEEPS its admin always-bypass, because the belt's direct pushes to the default branch depend on it; neither canon ruleset carries a `required_status_checks` rule. A ruleset is matched by its RULES and its BYPASS list, never by its name: a differently named ruleset with the same rules and an empty bypass list still covers, and a `main-guard` that lets an admin bypass it does not. Encoded in `templates/repo-settings.<kind>.json` and applied to an existing repo with `node scripts/new-repo.mjs --apply-settings <kind> --repo <owner>/<name> --only rulesets` (which also deletes the disabled, GitHub-created `Code Quality Copilot review for default branch` leftover); `node scripts/skeleton-check.mjs --settings` reports any repo that differs. A PRIVATE repo is N/A: rulesets answer 403 on the org's Free plan, measured 2026-09-24. Not adopted, by design: required signed commits, required linear history, required pull requests and merge gates on code or secret scanning, each of which would block the belt's direct pushes.
...
- **`persist-credentials: false` on a checkout that pushes nothing (prefer).** `actions/checkout` otherwise leaves the workflow token in `.git/config` for every later step, third-party actions included. Measured 2026-09-21: `scorecard.yml` and the private-working `gate.yml` set it; the other ten checkout-bearing template workflows (nine when measured; `create-release.yml` joined 2026-09-25) (`ci.yml`, `codeql.yml`, `coverage.yml`, `markdownlint.yml`, the article pair, `claude-ai-zips.yml`, `create-release.yml`, `deploy-check.yml`, `publish-pypi.yml`) do not yet, so a room adopts it through its own belt and the template follows once the flock rules on it.
- **`concurrency:` groups, MUST—one per workflow.** (`AGENTS.md`'s UMB-052 CANON CORRECTIONS entry, cited not restated: measured 7/7 rooms carrying ZERO `concurrency:` blocks before this ruling—the canon was silent, not the rooms drifting.) **The test is ATOMICITY, not read-vs-write**—a job that publishes something is not automatically never-cancel; the question is whether a run killed mid-flight leaves a PARTIALLY-APPLIED or corrupted external state behind, versus simply never publishing at all (a clean no-op). `cancel-in-progress: true`—a superseded run has nothing half-done to leave behind: `ci.yml` (reads/tests/compares, publishes nothing) · `codeql.yml` (a killed run simply never uploads its SARIF; the PRIOR analysis stands intact—no corruption, just a stale-but-coherent result) · `markdownlint.yml` (reads, publishes nothing) · `coverage.yml` (an upload is atomic per report—a killed run leaves the PRIOR report standing, stale but coherent, the `codeql.yml` class) · **`dependabot-auto-merge.yml`, CORRECTED (CoalWorks r29, main's ruling): grouped per-PR (`dependabot-auto-merge-${{ github.event.pull_request.number }}`), never per-ref or per-workflow—two different Dependabot PRs must not serialise behind each other, so a shared or per-workflow group is the wrong key regardless of the flag below it. WITHIN one PR's own group, `true` is correct: enabling auto-merge is IDEMPOTENT (the same atomicity discriminator this bullet already applies elsewhere), so a cancelled run corrupts nothing—the next push to that same PR simply re-enables it. A prior version of this row placed the same workflow under `false` reasoning from the shared-group case that never applies once the key is scoped per-PR; that reasoning is retired, not restated.** `cancel-in-progress: false`—a mid-flight kill would leave a partial or corrupted result: `scorecard.yml` (uploads a SARIF *and* posts to the OpenSSF API—unlike CodeQL's atomic upload-or-nothing, a killed Scorecard run can leave a partially-applied publish; it is also schedule-driven, so overlap is rare and queueing costs nothing) · `claude-ai-zips.yml` (a run killed mid-upload can leave a Release with a partial or missing ZIP asset and nothing to signal it—CoalMine's own prior `v3.17.1` incident, two overlapping runs racing onto the same tag, is the exhibit; grouped on the TAG/`ref_name`, not the branch, since the collision is per-Release) · `create-release.yml` (the same reason without the assets: a run killed mid-write leaves a Release whose title and body were never re-read). **Exemplar: CoalMine `a5cd9e6`+`2953998`**, all 6 workflows conforming to the shape above—`dependabot-auto-merge.yml` is the 7th, this room's own adoption per its CWK-058 pass, not fixed here.

A repo whose skills are published to claude.ai also ships the **claude.ai ZIP-packaging pair**—on a tag push it stages every skill with its description trimmed to claude.ai's 200-char cap, zips each staged directory, and attaches the ZIPs + a `SHA256SUMS.txt` to the GitHub Release as assets. **Copy it from [`templates/overlay-coal-skill/`](./templates/overlay-coal-skill/)** (`.github/workflows/claude-ai-zips.yml` + `scripts/build-claude-ai-zips.mjs`, reconciled 2026-09-03 per UMB-045 from the three live copies—CoalMine is the canonical source, named the flock exemplar in its own `MEMORY.md`; the retired `templates/zip-skills.yml` is tombstoned in `RULES-RETIRED.md`, never a live copy target). It performs NO adaptation, so **a skill folder must be self-contained** (a `SKILL.md` pointing outside its own folder fails the build rather than shipping a dangling instruction). Scope + the capability table: [CLAUDE-AI-INSTALL.md](./CLAUDE-AI-INSTALL.md). **Since UMB-162 (a) it is also the repo's SOLE Release creator**—deriving the title + body from CHANGELOG.md and re-reading what it published—and it prunes older stable Releases' ZIP assets (flag-gated, off by default) per [RELEASE-PATTERN.md](./RELEASE-PATTERN.md#write-once-derive-everything). Three rooms carry this pair today: CoalMine, CoalFace, CoalLedger (measured 2026-09-22, `GET /repos/{o}/{r}/contents/.github/workflows/claude-ai-zips.yml`). **A room that ships no ZIPs carries the bare `create-release.yml` from the same overlay instead**—the same derive, create and re-read steps with no packaging (a room carries exactly one of the two, never both; `scripts/template-workflows.test.mjs` holds their shared steps line for line). It copies `.github/workflows/create-release.yml`, `scripts/release-notes.mjs`, `scripts/verify-release-shape.mjs` and `scripts/lib/release-shape.mjs` byte-identical, with their three test files added to the room's own test roster. Until a room adopts one of the two, its head's REST post is a named interim, stated in the head's return.
...
**`coverage.yml`—a REPORT, never a gate (UMB-120).** It runs the test suite under Node's own coverage reporter, converts the lcov file to Cobertura XML with a converter that ships in the repo (`.github/scripts/lcov-to-cobertura.mjs`—no `npm install`, no run-time fetch), and uploads it with `actions/upload-code-coverage` (SHA-pinned like every action), so GitHub's built-in code coverage comments the line-coverage percentage and a per-file breakdown on the pull request. **NEVER a required check, and no threshold anywhere in it**—a coverage number nothing may block on is a diagnostic (`testing.md` §Coverage), and Node's coverage flag is Stability 1 EXPERIMENTAL. **FORCE: prefer** for an existing room (adopting it is the room's own pass; the template ships it, so a NEW repo carries it by construction), never a MUST on a number. It needs Code Quality switched on for the repository itself (a per-repo setting the template cannot carry); until a room's `scripts/test.mjs` exists the job skips cleanly, and an upload the API refuses (Code Quality off is the expected cause—read from the action's own README, not yet measured here) surfaces as an `::error::` annotation while the step exits 0 (`fail-on-error: false`), so it is never a red run. Measured on this org's Free plan, 2026-09-19: the upload was accepted and the bot commented on a pilot pull request.

**`.coderabbit.yaml`—a MEASURED TRIAL, not yet a canon file (AR-46, owner-signed 2026-09-21).** [`templates/published-code/.coderabbit.yaml`](./templates/published-code/.coderabbit.yaml) carries `reviews.profile: assertive` and `reviews.path_instructions` that restate this document's own contracts (hooks → Phoenix; scripts → zero-dependency and fail-loud; README/SECURITY/CONTRIBUTING → data verbatim from its source; workflows → SHA pins, `concurrency:` groups, `persist-credentials`). Each block names the rule it restates, and there is no `tone_instructions`: an instruction may encode a contract, never a preference. The vendor's own precedence rule (repository file over organization settings, but not over global overrides, and no merge without `inheritance: true`) is quoted in the file's header. `skeleton-check.mjs` does not list it as a skeleton file until the trial's measurement reads; a room adopts it through its own belt.
...
**`link-check.yml`, MUST, beside `markdownlint.yml`.** `markdownlint` checks markdown FORMAT; nothing checks that a link in it actually resolves. Zero-dep per Phoenix #2: a small `scripts/lib/link-check.mjs` walking the repo's own `.md` files (internal relative links + anchors) is the default; an external SHA-pinned action (`gaurav-nelson/github-action-markdown-link-check` or equivalent) is acceptable ONLY where it is pinned by 40-char SHA like every other workflow action here—never a bare `@vN` tag. It carries NO doc-only `paths-ignore`—markdown is its subject, exactly as with `markdownlint.yml`—and any `paths:` filter must still cover every file type a link can point at (CoalHearth runs with none for that reason; a `**.md`-only filter misses a renamed image or asset). **A filtered `link-check.yml` also cannot be a required status check** (GitHub, troubleshooting required status checks: a workflow skipped by path filtering leaves its checks "Pending" and blocks merging; "Avoid requiring workflows that can be skipped"), and the required context is the JOB's reported name (`check`, or `gate` in CoalMine), never the workflow's name (`link-check`)—so a room adds it to its gate ruleset only once it runs on every pull request; CoalHearth is the one room that does today.
...
7. Release + propagation per [scripts-quality.md](./scripts-quality.md); tags = beta + stable, GitHub Releases = stable-only plus the one launch-form pre-release Release of a repo's first beta/rc—every later beta/rc tag gets no Release; [RELEASE-PATTERN.md](./RELEASE-PATTERN.md) governs, not restated here.
