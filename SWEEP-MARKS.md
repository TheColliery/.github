...
| 9 | **CI + Code scanning: EVENT-DRIVEN since 2026-07-16 — the `alert-on-red.yml` net alerts on error; nobody polls per push** (USER: "เปลี่ยนจากเช็คทุกครั้งที่ push เป็น เช็คทุกครั้งที่ error"). A failed/timed-out CI or CodeQL run on main opens ONE `ci-red` issue (assignee → email); a NEW code-scanning alert turns the CodeQL run red via the fail-on-open-alerts step (an alert can no longer hide behind a green run; it also blocks Dependabot auto-merge until fixed/dismissed). The mark now = **confirm no open `ci-red` issue at release time** (one cheap look, not a poll); a local-green/CI-red split still usually means the PUSHED content differs from the working tree (the CT v1.2.0 pathspec incident). NET STATUS: deployed 2026-07-16, live-fire pending — until the net catches its first real red, a stable-tag release MAY still spot-check Actions once (Dependabot posture: fail-safe = worst case is the old behavior). | open `ci-red` issues (the net) · Actions spot-check on stable tags until first live catch |
