...
| 3 | SIGNED annotated tag pushed | `git tag -s vX.Y.Z` (`scripts-quality.md` §3) + `--follow-tags` | the room's department head |
| 4 | GitHub Release—STABLE tags only (beta = tag-only, except the ONE launch-form Release of a repo's first pre-release version, RELEASE-PATTERN.md). The notes carry the CHANGELOG entry's content; a gate receipt (test counts · VERIFY · CI) or a cross-repo pointer may ride along ONLY if verified at press | the tag-push workflow (`claude-ai-zips.yml`, or `create-release.yml` for a room with no ZIPs) | POSTER = the workflow: it derives the title + body from the CHANGELOG entry, creates the Release and re-reads it byte for byte ([re-read rail](RELEASE-PATTERN.md#a-release-body-is-re-read-after-every-write), UMB-050). DRAFTER = the doc-writer (the entry) · CHECKER = the reviewer (the entry against RELEASE-PATTERN, FIRST) · SIGNER = the department head, who presses the version bump and reads the posted Release back. A room with neither workflow: the head's REST post is a named interim, stated in its return, with the same re-read |
...
| # | Mark | Where | Owner |
|---|---|---|---|
| 1 | Org landing "Active Repositories" badge list (a beta room is marked `(beta)`; its Downloads/Developers badges are the ones `update-readme.mjs` rewrites, mark 3) | `.github/README.md` (the list lives in this root README; `profile/README.md` carries the suite table, mark 2) | `.github` deputy |
...
| 4 | Installer menu + install prose/table | `install.mjs` (the repo file to EDIT; `colliery-install.mjs` is only the local name the documented `curl -o` writes) + the install sections of both org READMEs (`README.md`, `profile/README.md`); a room with nothing installable is N/A here, stated not skipped | `.github` deputy |
...
| Ruleset `dependabot-auto-merge-gate` | active | branch ruleset on `~DEFAULT_BRANCH`, `required_status_checks` = the stable CI contexts (`all-green` + `analyze (javascript)` for a skill repo; `verify` for `.github`), `strict_required_status_checks_policy: false`, **bypass_actors = RepositoryRole Admin (actor_id 5, `always`)** so the maintainer's direct pushes have ZERO friction (`skeleton-check.mjs --settings` compares this list per repo since UMB-131—an org transfer once dropped it silently). **Each required check names its SOURCE: the GitHub Actions app (`integration_id` 15368), never "any source"** (owner AR-75 (3), 2026-09-25)—with the source open, any app installed on the repo could post a passing `all-green`; `--settings` reports an unsourced check as DIFFERS, and `scripts/gate-check-source.mjs` prepares the full PUT body per repo (dry run) and applies one repo per run on the owner's word. This makes `gh pr merge --auto` WAIT for green instead of merging instantly—it is load-bearing, not optional. `POST /repos/{o}/{r}/rulesets`. |
| Ruleset `main-guard` | active | branch ruleset on `~DEFAULT_BRANCH`, rules `deletion` + `non_fast_forward`, **bypass_actors = EMPTY** (rulesets canon, owner 2026-09-24; a deliberate history rewrite means the owner disables it in the UI first). Every PUBLIC repo; N/A on a private repo (403 on the Free plan). `node scripts/new-repo.mjs --apply-settings <kind> --repo <o>/<r> --only rulesets` applies it, `skeleton-check.mjs --settings` compares it (rules AND bypass list). |
| Ruleset `tag-immutable` | active | tag ruleset on `refs/tags/**`, rules `update` + `deletion` + `non_fast_forward`, **bypass_actors = EMPTY**; creating a tag stays free. Same apply and compare as `main-guard`. |
