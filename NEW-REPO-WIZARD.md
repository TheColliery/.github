...
7. **The git hooks are NOT installed by cloning—this is the step that gets skipped.** `git config core.hooksPath .githooks`, **then the proof step: plant a defect, watch the commit get REJECTED.** [`scripts-quality.md`](./scripts-quality.md) §2 measured **7 of 8 repos with ZERO hooks installed**—the "never commit with `--no-verify`" rail was protecting nothing on 7 of them. **Mechanism: NONE.** Nothing in the org today checks `core.hooksPath` is set on a repo; a fresh clone is silently ungated until this line is run by hand and nobody is warned. Named here as an unenforced rule, not fixed here. **And the hook must be committed EXECUTABLE (mode `100755`)**—git silently skips a non-executable hook on Linux and macOS, so a fence committed as `100644` does not exist there. The templates now ship `100755`, but a scaffold run on Windows writes files with no execute bit: after `git add`, run `git update-index --chmod=+x .githooks/*` (and read it back with `git ls-files -s .githooks`) before the first commit. Measured at the API 2026-09-21: Kolwen and Bankfire commit their hooks `100644` (the fence is absent on a POSIX clone), the seven Coal rooms and Chotmeter carry `100755`.
...
9. The repo's own `.github/`—`ISSUE_TEMPLATE/` (`bug-report.yml` + `config.yml`; add `platform-report.yml` when the tool is cross-agent) and, only where the tool ships to claude.ai, `marketplace.json` + the claude.ai ZIP-packaging pair (copied verbatim from [`templates/overlay-coal-skill/`](./templates/overlay-coal-skill/)—see `SKILL-REPO-PATTERN.md` Layer 5). Version-carrying issue-template placeholders get a `version-pin:` marker so `verify.mjs` catches drift.
...
11. **Per-repo workflows (copied INTO the new repo), all SHA-pinned 40-char with a `# vX` comment:** `ci.yml` · `codeql.yml` · `markdownlint.yml` · `scorecard.yml` · `coverage.yml` (report-only; the converter it runs, `.github/scripts/lcov-to-cobertura.mjs`, travels with it) · `dependabot-auto-merge.yml` + `dependabot.yml` → `SKILL-REPO-PATTERN.md` Layer 5. **Coverage also needs Code Quality switched on for the new repository (Settings → Security → Code quality)—a per-repo setting no template carries; until it is on, expect the upload to be refused (annotated, not red—`fail-on-error: false`).** **Verified today: `update-readme.yml`, `verify-landing.yml` live ONLY in `.github`'s own workflows—zero copies exist in any room repo (a third, `skillspector-version-watch.yml`, was retired on 2026-09-26).** These are org-level sweeps a new repo is covered by automatically once added to the org; copying them into a room repo would be the wrong move, not a missed one—naming this because the dispatch's own workflow list mixes both classes and a reader could conflate them.
...
17. **Repo settings—the one-time creation config** → [`SWEEP-MARKS.md`](./SWEEP-MARKS.md) §"GitHub repo settings—the creation-time setup," full table there (Issues/Discussions on, Projects/Wiki off, auto-delete-branch on, auto-merge on, the `dependabot-auto-merge-gate` ruleset, the `main-guard` and `tag-immutable` rulesets (empty bypass; `node scripts/new-repo.mjs --apply-settings <kind> --repo <owner>/<name> --only rulesets`), security-and-analysis all on, workflow-token read-only, default branch `main`, `FUNDING.yml`, Scorecard posture dismissals). **`MEMORY.md`'s "GitHub / org facts" already records what is enabled org-wide—read it before re-deriving, per its own instruction.**
