...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Developers-482%2B%20%2F%2014d-brightgreen?style=for-the-badge" alt="Developers">
...
| **CoalGob** | *Backfill* | ⚪ **Design** | An agent's `rm` goes **straight past the Recycle Bin**; yours does not. A person can bypass the bin but rarely does—an agent does it on **every** delete, by default, because the shell path was never wired to it. CoalGob would route an agent's deletes through the operating system's **own** trash (`IFileOperation` · `NSFileManager.trashItem` · `gio trash`/`trash-cli`) and **refuse rather than fall back** where no such mechanism exists—so an agent's mistake is as recoverable as a person's. The safety net belongs to the OS, which means it still works when the tool's own logic is wrong. Named 2026-08-01; a **lite form already ships inside CoalWash**. **A design record only—nothing built, nothing to install.** |
