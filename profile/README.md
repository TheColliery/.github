...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Downloads-2.1k%2B%20%2F%2014d-orange?style=for-the-badge" alt="Downloads">
  <img src="https://img.shields.io/badge/Developers-637%2B%20%2F%2014d-brightgreen?style=for-the-badge" alt="Developers">
...
| **[CoalWash](https://github.com/TheColliery/CoalWash)** | *Washing* | **Live** ![ver](https://img.shields.io/github/v/tag/TheColliery/CoalWash?sort=semver&label=&color=success&style=flat-square) | A **memory washer/defragmenter** with **two lanes and two different contracts**. **Class-B—the context you load every session** (memory + governance files): cleans the FAT, never the MEAT, under a **code-enforced fidelity gate proving zero structured-token loss by diff**, with **plan-sourced deletes made safe by UNDO** (verified snapshot + WAL + whole-run rollback, not pre-approval; pinned files untouchable), and a session-start gauge whose FULL band **force-runs** the free mechanical pass on a **deterministic break-even proof with the numbers shown**. **Class-A—the at-rest estate on disk** (transcripts and tool results): **never semantic-edited at all**—they are vendor-owned and machine-parsed, so the guarantee is **byte-identity**, copy-verify-then-delete with a round-trip restore. The estate lane runs **only when you choose it in the wizard**—never on a band, never ambient, because disk is not context. Cross-agent design (Claude Code validated). |
...
| **[CoalGob](https://github.com/TheColliery/CoalGob)** | *Backfill* | **Beta** ![ver](https://img.shields.io/github/v/tag/TheColliery/CoalGob?include_prereleases&sort=semver&label=&color=orange&style=flat-square) | An agent's `rm` goes **straight past the Recycle Bin**; yours does not. A person can bypass the bin but rarely does—an agent does it on **every** delete, by default, because the shell path was never wired to it. CoalGob is designed to check each destructive command an agent runs for whether it already lands somewhere recoverable (the operating system's **own** trash, or a destroyer that already snapshotted it), and to block it only when it would not—so an agent's mistake is as recoverable as a person's. **Honest state, in beta: only the first piece ships**—a zero-dependency **command classifier** that reads a shell command and returns `DESTRUCTION` / `OUT_OF_SCOPE` / `NO_MATCH`, with its test suite. **There is no hook and no interception yet, so nothing is blocked or rerouted today, and `NO_MATCH` is never a safety claim; nothing to install.** A lite form of the idea already ships inside CoalWash. |
| **CoalSeam** | *The Seam* | ⚪ **Capture** | The **org system itself**, being captured as a skill. Claude Code persists the *main* agent's memory automatically; a spawned worker gets nothing and re-learns its domain every time. CoalSeam gives each **role** that same persistence—a code / docs / review worker becomes one continuous identity across spawns—and, the sharper half, gives the rings **below** the main a **split load**: the constitution binds the main every turn, but a doc worker never needs the containment rules and a code worker never needs the doc manual. The platform offers all-or-nothing there; the middle is where every worker lives. **No new platform, no extra vendors, one model—the org is the runtime's own primitives, so it cannot drift from them.** Honest state: a captured skeleton plus a design record. **Not a plugin, not yet a repository, nothing to install.** Start method, activation and per-user generalization are all deliberately unsettled. |
...
| **CoalBoard** | solo-vs-board (2026-07-03, v1.5.5): **Opus 4.8** solo 4/5 · board 5/5 (strong solo catches the reasoning traps unaided; board's edge = the version-sensitive FACT T3). **Cross-vendor mirror**—Gemini 3.5 Flash solo ~4/15 · board 5/5 (weak solo → board recovers ALL traps). Board margin scales inversely with solo strength; T3 (run-the-check) is the win at both ends. Board = solo + ground-truth execution. **Third-party comparison (2026-09-16, CoalBoard v2.4.2 vs `cloudflare/security-audit-skill` commit `c1c8a8c`, n=3, model ids in the record, disclosure: we build one of the three arms)**—CoalBoard's audit mode vs the skill's quick profile vs a solo control on one seeded target: skill recall matched the solo control (0.467 median) at ≈14.5× its median cost; CoalBoard reached 0.867 median recall at ≈3.9× less median cost than the skill. **Addendum 1 (the judge chair, measured 2026-09-16):** judge seat at the weakest model tier over strong-tier lenses (two research-only levers, not shipped) held on retention—38 of 39 lens-union seeds kept, 0.867 recall at ≈0.947 strict precision that is largely the strong lenses' own; whether a weak judge rejects a look-alike stays untested, and the whole-weak-tier board read "in between"—[see the record](https://github.com/TheColliery/.github/blob/main/benchmarks/CoalBoard/RESULTS.md) |
...
| **SkillDeterminism** | the second non-tool benchmark (2026-07-31 to 2026-08-03): do independent readers extract the SAME directives from the SAME shipped `SKILL.md` prose, repeated reads across weak/mid/strong tiers · two high-index skills (CoalBoard ~83, an un-indexed hook-injected cue) FAILED—one hit 67% divergence on its enumeration rail with the top tier split against itself, another reached 0% variance while unanimously WRONG (agreement ≠ correctness, always scored separately) · the one low-index skill walked (CoalMine `rot-canary`, index 12.0) PASSED its 4 pre-registered rails at 0% variance, with one small un-predicted 33.3% wobble traced to a single un-enumerated conditional—[see the record](https://github.com/TheColliery/.github/blob/main/benchmarks/SkillDeterminism/RESULTS.md) |
...
1. 🌐 **Works in Every Mine (Cross-Agent):** Vendor-agnostic by design, and each tool says exactly how far it has been taken—no blanket claim covers them all.
   ...
   | **CoalFace** | **Works with** every concurrent-subagent platform (Antigravity, Cursor, Codex, Gemini CLI, Cline, Copilot)—the swarm is e2e-proven only on Claude Code; re-verify on yours. |
   | **CoalTipple** | **Claude Code only**—Antigravity's `invoke_subagent` DOES let an agent pick a per-spawn model tier, but that tier selects a cross-vendor Google model with no effort knob, which doesn't fit CT's two-knob (tier×effort) design: a different, unbuilt product, not a missing spawn param. Other platforms are under monthly review. |

   Headline tiers are exactly two words, per [DOC-PATTERN.md](https://github.com/TheColliery/.github/blob/main/DOC-PATTERN.md#platform-tiers-row-4-locked-2026-08-01): **validated** = we built it, test it, run it here daily · **works with** = it installs and runs, documented, not yet run by us. Neither is a promise; a `works with` row becomes `validated` only after a real run. CoalTipple carries no `works with` row because it does not run on any other platform at all—the cell states why, not a tier.
