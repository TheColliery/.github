...
| **CoalWash** | *Washing* | 🔵 **Blueprint** | A **memory-defragment engine** (the 6th sibling, in design): cleans + compacts an agent's persistent memory — notes **and** governance — under a hard **zero-fact-loss** guarantee (a code-core scout → gate → atomic-apply pipeline, with a 2-agent outsider+insider judgment and human-gated deletes). Cross-agent. *Spec in design; build deferred.* |
| **CoalLedger** | *Records* | 🔵 **Blueprint** | A **docs-health canary suite** (the 7th sibling, in design): the docs-side mirror of CoalMine's code canaries — grounding (docs match their source-of-truth), completeness, rot, consistency, structure (AST rendering-fidelity), quality (incl. language mechanics), + an optional leak check. Cross-agent + **multilingual**; the engine reuses CoalMine. *Spec in design; build deferred.* |
...
node colliery-install.mjs 4 5       # CoalHearth + CoalFace
