...
| Docs spine | Full [DOC-PATTERN.md](./DOC-PATTERN.md) spine (README/SECURITY/CONTRIBUTING/CODE_OF_CONDUCT/PRIVACY). The README's own rows split the same way: an L1 floor every published-code repo carries, and the L2 rows (`Commands`, `Permissions`, `Benchmark`) that belong to the Coal* skill overlay—a non-skill repo declares each of those N/A in one line, never leaves it silent | None—LICENSE only | LICENSE + CONTRIBUTING + CHANGELOG |
| GOVERNANCE | MUST: every public repo states its governance—who decides, how a change is accepted, who writes the changes (AI agents under the maintainer's review, the `Co-Authored-By` trailer as the record)—either inherited from the org-default `GOVERNANCE.md` or in its own file of the same shape (OpenSSF Best Practices, silver: governance) | N/A—a private working repo has no public reader | Its own `GOVERNANCE.md` (the template ships the placeholder) |
| `.gitattributes` | MUST, every series repo, existing as well as new (Git gitattributes(5): line endings are a repo property, never a per-machine `core.autocrlf` setting). The shape is NEW-REPO-WIZARD.md step 4: `* text=auto eol=lf` · `/.githooks/** text eol=lf` · `*.ps1`/`*.cmd`/`*.bat` `text eol=crlf`. A divergence is named inside the file with its reason. Re-derive: `md5sum */*/.gitattributes` from the umbrella | Same shape; `templates/private-working/` ships it since UMB-232 | Same shape |
...
| Typography | Public-class prose (README, docs, SECURITY, and the like) uses an unspaced em dash—like this—for a parenthetical break, never a spaced dash or an ASCII double hyphen (DOC-PATTERN.md, owner ruling 2026-09-03) | N/A by declaration—a private repo's own prose is not held to a public presentation convention | Same as `published-code`—public-class prose uses the unspaced em dash |
