...
<!-- version-transition: SkillSpector scan — the re-scan is automatic on two events (E1: a new SkillSpector version, opened by the org's weekly watcher; E2: this repo's own version bump, gated by a per-repo baseline diff); a genuinely new attack surface is a third, by hand. Record the version/score/date/commit here only after a real scan. -->
